...
local version_err_msg = 'ERROR: pandoc >= ' .. required_version .. ' required for columns filter'
...
local function remove_class(element, class)
  ...
  if element.attr and element.attr.classes then
    ...
    if element.attr.classes:includes(class) then
      element.attr.classes = element.attr.classes:filter(function(x)
        return not (x == class)
      end)
    end
  end
...
local function add_to_html_style(element, style)
  ...
  if element.attr and element.attr.attributes then
    ...
    if element.attr.attributes['style'] then
      element.attr.attributes['style'] = element.attr.attributes['style'] .. '; ' .. style .. ' ;'
...
local function ensures_latex_length(text)
  ...
  for _, unit in pairs(units) do
    ...
    if string.match(text, '^%.%d+' .. unit .. '$') or string.match(text, '^%d+%.?%d*' .. unit .. '$') then
      result = text
    end
  end
...
local function process_meta(meta)
  ...
  if FORMAT:match('latex') then
    return add_header_includes(meta, pandoc.RawBlock('latex', '\\usepackage{multicol}\n'))
  end
...
local function convert_explicit_columbreaks(elem)
  ...
  if
    ...
    and elem.content[#elem.content].classes:includes('column')
  then
    elem.content[#elem.content] = add_class(elem.content[#elem.content], 'column-div-in-last-position')
  end
...
local function tag_with_number_of_explicit_columnbreaks(elem)
  ...
  local filter = {
    ...
    Div = function(el)
      if el.classes:includes('columnbreak') and not el.classes:includes('columnbreak_already_counted') then
  ...
  elem = set_attribute(elem, 'number_explicit_columnbreaks', number_columnbreaks)
  ...
  if elem.attr and elem.attr.attributes then
    ...
    -- otherwise fetch and unset any alias
      ...
      if elem.attr.attributes['columnsep'] then
        elem = set_attribute(elem, 'column-gap', elem.attr.attributes['columnsep'])
        elem = set_attribute(elem, 'columnsep', nil)
      end

      if elem.attr.attributes['column-sep'] then
        elem = set_attribute(elem, 'column-gap', elem.attr.attributes['column-sep'])
        elem = set_attribute(elem, 'column-sep', nil)
      end

      if elem.attr.attributes['columngap'] then
        elem = set_attribute(elem, 'column-gap', elem.attr.attributes['columngap'])
        elem = set_attribute(elem, 'columngap', nil)
      end
    end
    ...
    -- otherwise fetch and unset any alias
      ...
      if elem.attr.attributes['columnrule'] then
        elem = set_attribute(elem, 'column-rule', elem.attr.attributes['columnrule'])
        elem = set_attribute(elem, 'columnrule', nil)
      end
    end
  end
...
local function determine_column_count(elem)
  ...
  if elem.attr.attributes and elem.attr.attributes['column-count'] then
    specified_column_count = tonumber(elem.attr.attributes['column-count'])
  end
  ...
  if elem.attr.attributes and elem.attr.attributes['number_explicit_columnbreaks'] then
    number_explicit_columnbreaks = tonumber(elem.attr.attributes['number_explicit_columnbreaks'])
...
local function header_to_latex_and_inlines(header)
  ...
  if header.level and latex_header[header.level] then
    inlines:insert(1, pandoc.RawInline('latex', '\\' .. latex_header[header.level] .. '{'))
    inlines:insert(pandoc.RawInline('latex', '}'))
  end

  -- wrap in a link if available
  if header.identifier then
    inlines:insert(1, pandoc.RawInline('latex', '\\hypertarget{' .. header.identifier .. '}{%\n'))
    inlines:insert(pandoc.RawInline('latex', '\\label{' .. header.identifier .. '}}'))
  end
...
local function format_colspan_latex(elem, number_columns)
  ...
  if #elem.content == 1 and elem.content[1].t == 'Header' then
    ...
    inlines:insert(pandoc.RawInline('latex', '\\end{multicols}\n'))
    inlines:insert(pandoc.RawInline('latex', '\\begin{multicols}{' .. number_columns .. '}['))
    ...
    result:insert(pandoc.RawBlock('latex', '\\end{multicols}\n'))
    result:extend(elem.content)
    result:insert(pandoc.RawBlock('latex', '\\begin{multicols}{' .. number_columns .. '}'))
    return result
  end
end
...
local function format_columns_latex(elem)
  ...
  if
    elem.classes:includes('ragged')
    or elem.classes:includes('raggedcolumns')
    or elem.classes:includes('ragged-columns')
  then
    ragged = true
  elseif
    elem.classes:includes('justified')
    or elem.classes:includes('justifiedcolumns')
    or elem.classes:includes('justified-columns')
  then
    ragged = false
  end
  if ragged then
  ...
  if elem.attr.attributes then
    if elem.attr.attributes['column-gap'] then
      local latex_value = ensures_latex_length(elem.attr.attributes['column-gap'])

      if latex_value then
        latex_begin = latex_begin .. '\\setlength{\\columnsep}{' .. latex_value .. '}\n'
      end
    ...
    if elem.attr.attributes['column-rule'] then
      ...
      local latex_values = css_values_to_latex(elem.attr.attributes['column-rule'])

      if latex_values['length'] then
        latex_begin = latex_begin .. '\\setlength{\\columnseprule}{' .. latex_values['length'] .. '}\n'
      end

      if latex_values['color'] then
        latex_begin = latex_begin .. '\\renewcommand{\\columnseprulecolor}{\\color{' .. latex_values['color'] .. '}}\n'
      end
  ...
  latex_begin = latex_begin .. '\\begin{multicols}{' .. number_columns .. '}\n'
...
local function format_columns_html(elem)
  ...
  if elem.attr.attributes then
    if elem.attr.attributes['column-gap'] then
      elem = add_to_html_style(elem, 'column-gap: ' .. elem.attr.attributes['column-gap'])
    ...
    if elem.attr.attributes['column-rule'] then
      elem = add_to_html_style(elem, 'column-rule: ' .. elem.attr.attributes['column-rule'])
...
syntactic_sugar_filter = {
  ...
  Div = function(element)
    ...
    for _, class in pairs(element.classes) do
      ...
      local number = number_by_name(string.match(class, '(%a+)[_%-]?columns$'))

      if number then
        element = set_attribute(element, 'column-count', tostring(number))
    ...
    if element.classes:includes('column-span') or element.classes:includes('columnspan') then
